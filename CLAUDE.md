...
Расчётный поток: леджер → нормализованные строки (`ledger`/`fx`/`engine`) →
метрика в DSL (`dsl`/`interp`/`templates`) → вердикт → улика (`evidence`) →
лестница фолбэков (`fallbacks`). Документный поток (пока не подключён к solve,
подключается задачей 24): PDF → `pdftext`/`vision` → `route` → `dossier` →
`facts_extract`/`specs_extract`.
...
- **`solve.py`** — harness: `main(archive)` печатает `dataset_hash` первой
  ...
  ячейки. Ядро — `run_cell`; спеки и факты пока эталонные
  (`legacy_spec_to_cellspec` + `eval/expected_extraction.py`), задача 24
  подменяет их на извлечённые. Трейсы: `work/<hash>/trace/<сценарий>.<пункт>.json`
  (имя разбирать как `stem.split(".", 1)`) и `<сценарий>.borrower.json`.
- **Документный слой:** `pdftext.py` (постраничный текст, слепота = <200
  ...
  и без LLM), `dossier.py` (действующая редакция: маркер перебивает дату),
  `facts_extract.py` (факты с цитатами; непроверяемая цитата = отброс факта).
...
- `FACTS` и `SPECS` живут в `eval/expected_extraction.py` как эталон для
  измерения LLM-слоя; solve читает их только через мост, который задача 24
  подменит на извлечённые артефакты.
