...
def _extract_number_formats(num: Decimal | int | float) -> set[str]:
    ...
    Форматы 9.00, 9.0, 500000, 500_000, 4_000_000, а также разрядные
    разделители из текстов договоров: 4,000,000, «4 000 000» (пробел и NBSP).
    Голые целые добавляются только при > 999 — иначе ложные срабатывания на
    маленьких числах. ROUND_HALF_UP — как в score.py. Округлённые результаты,
    совпавшие с весами скоринга, пропускаются.
    """
    formats = set()
    ...
    if d == d.to_integral_value():
        ...
        if int_val > 999:
            formats.add(int_str)
            # Разрядные группы с разделителями: 4_000_000 (код), 4,000,000 и
            # «4 000 000» / NBSP (текст договора, попавший в промпт или строку)
            if len(int_str) > 3:
                groups = [int_str[max(0, i - 3) : i] for i in range(len(int_str), 0, -3)][::-1]
                for sep in ("_", ",", " ", "\N{NO-BREAK SPACE}"):
                    formats.add(sep.join(groups))
        elif int_val > 0:
