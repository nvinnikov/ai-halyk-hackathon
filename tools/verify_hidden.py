"""Проверка, что закрытый архив у вас — тот самый, на котором считались цифры.

Самого архива в репозитории нет и не будет: он принадлежит организаторам
Halyk AI Challenge, а не нам. Но все числа разбора привязаны к конкретным
байтам, поэтому проверка их идентичности должна быть в репозитории — иначе
воспроизвести разбор нельзя, а поверить на слово нечему.
