...
| --- | --- | --- |
| `extracted` — **боевой**, дефолт `main()` и `run.sh` | **34.50 / 36.00 (95.8%)** | `tests/test_extracted_gate.py`, порог `EXTRACTED_BASELINE` |
...
`eval/expected_extraction.py`. Боевой режим сейчас **выше** эталонного, и это не
парадокс: часть просадок сидела не в извлечении, а в том, что расчёт слушался
недействующих редакций документов — от этого одинаково страдали оба режима, а
починка досье лечит только тот, который документы действительно читает.
...
| `solution/pdftext.py`, `vision.py`, `route.py`, `dossier.py`, `facts_extract.py`, `specs_extract.py` | Документный слой: постраничный текст, vision по слепым страницам, маршрутизация, сшивка досье с выбором действующей редакции, факты и спеки с цитатами. |
...
| `docs/ops/` | Разборы прогонов: `remaining-losses.md` (корни оставшихся 1.50), `recovery-playbook.md` (что делать, если прогон сломался). |
