...
**Правки по ревью (обязательны):**
- Ожидание слепых страниц в тесте: с правилом «И» (задача 18) на публичном наборе их **8** — в тесте `assert s["blind_pages"] == 8` вместо `>= 4` (число проверено прямым замером по 200 PDF; «9» в ранней редакции этого пункта было опиской при переносе замера — см. комментарий в `tests/test_sanity.py`. Если реализация даст другое число — разбирать детектор, не подгонять тест).
