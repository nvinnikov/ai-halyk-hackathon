...
- Порядок жертв при отставании (пересмотрен по замеру research-дока, раздел 7): мутации → LOBO → **vision** → библиотека шаблонов. Библиотека уходит последней: 4 шаблона закрывают 21/36 ячеек, «голый DSL» нужен ради 15 одиночек. Инварианты, фолбэки, скелет-первым не жертвуются.
...
- Produces:
  ...
  - `fallbacks.prior_status(prior, direction: str | None, family: str | None) -> tuple[str, bool]` — `(status, is_conditional)`; безусловная глобальная доля → `is_conditional=False` (ячейка — подбрасывание монеты, алярм);
...
Из спеки, раздел 8, с пересмотром по замеру research-дока (раздел 7): если фундамент (задачи 1–9) не готов к концу 6 августа — потоки не стартуют, фундамент доделывается утром 7-го, из плана вылетает по порядку: задача 28 (мутации) → 29 (LOBO) → 19 (vision) → 15-шаблоны как отдельный ярус. Библиотека шаблонов режется последней, а не третьей: `related_abs` ×7, `capex` ×5, `revenue` ×5, `related_share_revenue` ×4 закрывают 21 ячейку из 36 дёшево и надёжно, «голый DSL» нужен ради пятнадцати метрик-одиночек (оговорка: доля посчитана по числу ячеек, веса по сложности неизвестны и у типовых наверняка ниже). Задачи 17 (фолбэки), 26 (инварианты) и скелет-первым (9) не жертвуются ни при каких условиях.
