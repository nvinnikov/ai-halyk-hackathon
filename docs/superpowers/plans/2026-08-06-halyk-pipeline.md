...
- **Модель.** `MODEL = "claude-haiku-4-5-20251001"` — подтверждён замером (research-док, разделы 5–16): 36/36 спек, 12/12 фактов, vision читает таблицы дословно, полный прогон $1.49, лимиты не ограничитель. Сравнение с opus/sonnet снято — haiku достаточно и дешевле всех. Константа меняется первым шагом задачи 11.
...
- Модель: `claude-haiku-4-5-20251001`, одна константа `MODEL` в `solution/llm.py`. Замер (research-док, разделы 5–16) закрыл вопрос: haiku даёт 36/36 спек и 12/12 фактов, vision читает таблицы дословно, полный прогон $1.49, лимиты (10k RPM, 10M ITPM) не ограничитель. `llm.py` сейчас на master с `MODEL="claude-sonnet-5"` — сменить на haiku первым шагом задачи 11 (одна строка; тесты мокают `_create`, поведение не меняется); опция сравнить с opus на репетиции снята — haiku достаточно.
...
>
> **Замер (research-док, разделы 5–12) подтвердил: ярус обязателен, не факультативен.** Правила первого яруса покрыли 98.4% незнакомых строк, но весь непокрытый остаток оказался систематическим видом операции, а не шумом: все 13 промахов — «Sewer discharge levy» (суммы до 6 млн), которого у целевых заёмщиков публичного набора нет случайно. LLM-ярус классифицировал все 11 уникальных описаний верно (UTILITIES). Плюс 16% формулировок у каждого заёмщика не имеют прецедента у остальных — рассчитывать на полноту правил на приватном наборе нельзя.
...
**Правки по замеру (обязательны; research-док, разделы 13–15):**

4. **Матчинг имён — `tokens()` обязан чинить три дефекта старого `norm()`; закрепить тестами.** Старый `norm()` ломал порядок операций (точки вырезались раньше regex юрформ): `norm('Aktau Holdings L.L.P.')='aktau holdings l l p'`, а `norm('Aktau Holdings LLP')='aktau holdings'` — не совпадало, P1 спасал только подстрочный матч, который спека 4.1 велит убрать. Плюс `norm('LLP')==''` и 130 из 455 ключей склеивали разные написания (`LLC JSC`). Новый `tokens()` (split по не-alnum → фильтр len≥3 и юрформ) это чинит: обе формы `L.L.P.`/`LLP` дают `{'aktau','holdings'}`, а `LLC JSC`→`{}`→`is_related` возвращает False (пустые токены не матчатся). Обязательные тесты в `test_engine.py`: `is_related('Aktau Holdings L.L.P.', ['Aktau Holdings LLP'])` True; `is_related('Ashford Office Supplies LLC', ['LLC JSC'])` False; `is_related('X', ['LLP'])` False. Подстрочного матча (`in`) в `is_related` быть не должно — только подмножество непустых токенов.
5. **`sign: net` — дефолт остаётся `out`, но с поячеечной диагностикой.** Замер: у целевых 153 положительные строки в расходных категориях (477 млн), но `out` и `net` дают идентичный результат — сторно лежит в MARKETING/RENT/INSURANCE/TAX/TELECOM, а в читаемых метриками OPEX/INTEREST/PAYROLL/UTILITIES разница ровно нулевая. `out` проверен на 35 ячейках, `net` — ни на одной. Дефолт для расходных категорий — `out`. Диагностика (в трейс ячейки, задача 17): для каждой метрики, читающей расходную категорию, посчитать значение и с `out`, и с `net`; при расхождении — поле `sign_divergence` в трейсе. На публичном наборе не сработает ни разу, 9 августа сразу покажет ячейки, где знак решает. (В `agg` оба знака уже реализованы — здесь только диагностический двойной расчёт.)
6. **`related_payments`/кандидаты улики — только расходные строки (`amt < 0`); закрепить тестом на B4.** Замер: у B4 связанная сторона `Shymkent Fuel Distributors LLP` даёт ~9.02 млн **поступлений** (4 строки продаж, все положительные). Если related-логика возьмёт их, `related_abs` B4 подскочит на 9 млн и 6.3 сломается. Обязательный тест `test_engine.py`: related-сторона с положительными строками не вносит вклад в `related_payments` (только `amt < 0`).
7. **Исправление эталона `eval/expected_extraction.py` (проверено на числах — гейт 34.00 устоит).** B4 `related_parties` неполон: `Shymkent Fuel Distributors LLP` — связанная сторона (48.0% голосов при пороге 30.0%), но в эталоне её нет. Добавить её к `["Kazyna Capital LLP"]`. Безопасно: все 4 строки Shymkent у B4 — поступления, а `related_abs` считает только расход, поэтому B4 6.3 остаётся 303 949.09 COMPLIANT и публичный скор не меняется (проверено обходом леджера). Это чинит extraction eval (задача 25): без правки он показывал бы 11/12 и штрафовал LLM-слой за верное извлечение. **Гейт-проверка:** после правки прогнать `tests/test_solution.py` — скор обязан остаться ровно 34.00. Правка идёт тем же коммитом, что перевод числовых фактов на строки (шаг 5 согласования).
...
period()/quarter() — ОТЧЁТНЫЙ период метрики (за какой интервал считать), НЕ
исключение операций. Отсечение периода / переход рисков — документальное
решение (excluded_transactions в фактах), применяется до DSL; замер: обе
исключаемые строки датированы серединой 2025 года, фильтром их не выбросить.
"""
...
- Produces:
  ...
  - `templates.match_signature(node) -> str | None` — имя шаблона по канонической сигнатуре (оставить как есть — резервный путь);
  - **`templates.match_heading(heading_key: str) -> str | None`** (правка по замеру, research разделы 5–12; ОСНОВНОЙ путь матча): заголовок пункта однозначно определяет метрику — 19 уникальных заголовков на 36 ячеек при 19 метриках, распределение один-в-один, тогда как тела пунктов почти все разные (31 скелет из 36), так что матч по нормализованному DSL-выражению ненадёжен. Матчить по языконезависимому ключу заголовка. Собрать `TEMPLATE_HEADINGS: dict[str, str]` — нормализованный ключ заголовка → имя метрики, где ключ строится языконезависимо (пять заголовков из девятнадцати английские при русском теле): нормализация = опустить регистр, схлопнуть пробелы, отбросить пунктуацию и числа. `specs_extract` (задача 23) отдаёт ключ заголовка каждой спеки; в solve (задача 24) сначала `match_heading(heading_key)`, при промахе — `match_signature(node)`, при промахе — сырой DSL. Тест: 19 заголовков дают 19 различных ключей; ключ языконезависим (английский заголовок при русском окружении матчится).
...
5. **Отчёт покрытия категоризации — в прогон, а не только в инварианты:** `scenario_inputs` зовёт `taxonomy.coverage_report(rows, referenced)` и кладёт результат в `<scenario>.borrower.json`; алярм `warn`/`critical` печатается в лог прогона. **Защита (found by review задачи 10):** `coverage_report` кидает `KeyError` (через `expand`), если в `referenced` пришло имя вне таксономии — падение диагностики уронило бы ячейку. Перед вызовом отфильтровать `referenced` через `taxonomy.is_category` (или обернуть вызов в тот же `try`, что и расчёт ячейки). То же для потребителя в задаче 26 (`check_other_share`) и задаче 30 (sanity).
...
**Правка по замеру (обязательна; research-док, разделы 5–12): отсев по типу, а не только по счёту.** Замер вскрыл обратный случай: `ffb902ce0a5b.pdf` содержит `ACC-7804`, но это методичка комплаенса, а не документ клиента — привязывать её нельзя. Правило: документ с `doc_type == "other"` (не agreement/kyc/audit_report/treasury_memo/financial_notes) НЕ привязывается к счёту даже при единственном целевом кандидате — он уходит в `quarantined` с причиной `non_client_doc_type` (без алярма-паники, это ожидаемый шум). Тип определяется LLM до привязки, поэтому META-вызов (первая страница, правка 4) делается раньше решения о привязке. Тест: документ типа `other` с единственным целевым ACC → не привязан, `quarantined: true`, reason `non_client_doc_type`. Симметрично сохраняется правило «нет ACC → карантин, не отброс» (KYC-скан `f3fa6d20c8a1.pdf` без ACC в текстовом слое).
...
**Правка по замеру (обязательна; research-док, разделы 5–12):** редакционная фильтрация применяется ТОЛЬКО к перевыпускаемым целиком типам — `agreement` и `kyc`. Документы типов `audit_report` (согласованные процедуры / примечания) и `treasury_memo` **кумулятивны**: каждый несёт своё документальное решение (реклассификация, исправление суммы), и отбрасывать их по дате — значит терять решения. Для кумулятивных типов в досье попадают ВСЕ документы (никакого `_pick_active`), их факты сливаются в задаче 22. Множество кумулятивных типов вынести константой `CUMULATIVE_TYPES = frozenset({"audit_report", "treasury_memo", "financial_notes"})`; `_pick_active` вызывается только для типов вне него. Тест: два audit_report с разными датами на один счёт → оба в `docs`, ни один не в `rejected`; два agreement → действует один, второй в `rejected`.
...
**Правка по замеру (обязательна; research-док, разделы 5–12): связанность считает код, а не модель.** Не спрашивать LLM «кто связанная сторона» — это перекладывает на модель порог и арифметику, которые детерминированны. Вместо этого KYC-промпт запрашивает по каждому контрагенту **долю голосующих прав** (`voting_share`, число) и **порог связанности из KYC** (`related_threshold`, отдельным полем) с цитатами. Связанность вычисляется кодом: контрагент связан ⇔ `voting_share >= related_threshold`. Из этих полей код собирает `related_parties` (список тех, кто прошёл порог) для обратной совместимости контракта, но `facts` дополнительно несёт сырые `voting_shares: {counterparty: "0.334"}` и `related_threshold: "0.30"` с цитатами — это и трейс, и вход инварианта. Аналогично для необременённых дочек, если порог задан долей. Тест: контрагент с долей 0.334 при пороге 0.30 → в `related_parties`; с долей 0.28 → нет; порог и доли лежат в артефакте с цитатами. **Порог у каждого заёмщика свой** (замер: двенадцать разных — 0.20, 0.25, 0.30, 0.32, 0.34, 0.35, 0.36, 0.38, 0.40), поэтому `related_threshold` — произвольный Decimal из документа, никаких зашитых 0.30; `voting_share`/`related_threshold` хранить строками (точный Decimal при сравнении).
...
**Правки по замеру (обязательны; research-док, разделы 5–12):**
1. **Период действия ковенанта — НЕ триггер.** Замер: haiku заполнил `trigger` строкой «период с 2025-01-01 по 2025-12-31» в 12 ячейках из 36; ложный триггер, вычисленный в False, отменил бы тест на трети ячеек. В `SPECS_PROMPT` явно: «`trigger` заполняется ТОЛЬКО когда применение теста зависит от значения показателя (пример: „если поступления по финансированию превышают $4 000 000“ — springing-ковенант). Срок действия договора, отчётный период, дата — НЕ триггеры, для них `trigger: null`.»
2. **Валидация триггера мягкая, не роняет ячейку.** Если `trigger` не парсится как `Cmp` двух DSL-выражений (`dsl.parse` → не `Cmp`, или `DslError`) — трактовать как `trigger: null` + алярм `trigger_discarded`, а НЕ помечать всю спеку `valid: false`. Метрика и порог валидны сами по себе; кривой триггер не должен стоить ячейку. Тест: спека с `trigger="период 2025"` → `valid: true`, `trigger` в артефакте `null`, алярм `trigger_discarded`.
3. **Матч шаблона — по языконезависимому ключу заголовка, а не по DSL** (усиление, детали в задаче 15): `specs_extract` дополнительно возвращает нормализованный ключ заголовка пункта (из `quote` или отдельного поля) для сигнатурного матча.
...
1. **Нормализация номера пункта (обязательна — замер мутаций показал 7/12 заёмщиков с нестабильным форматом).** Модель возвращает `clause` то как `"7.2"`, то как `"Пункт 7.2"` / `"п. 6.1"` / `"Article 6.1"` — на исходных номерах 1 случай из 36, на мутированных (clauses 6.x→7.x) — 7 заёмщиков из 12. Без нормализации harness, ищущий ячейку точным совпадением ключа, отправит больше половины ячеек приватного набора в фолбэк. В `extract_specs` нормализовать: `m = re.search(r"\d+(?:\.\d+)*", sp["clause"]); clause_key = m.group() if m else sp["clause"]`; несопоставимый с шаблоном пункт — алярм `clause_unmatched` (матч с ячейками шаблона — в solve, задача 24). Тесты (обязательны, все формы из замера): `covenant(clause="Пункт 7.2")`→`clauses["7.2"]`; `"п. 6.1"`→`"6.1"`; `"Article 6.1"`→`"6.1"`; `"6.1."`→`"6.1"`.
...
8. **Новый инвариант `check_dirty_rows_recovered`** (правка по замеру, research разделы 13–15). Замер: единственный реальный вид грязных данных — пустая сумма (2 строки, обе у целевых, обе восстанавливаются записками казначейства через `amount_override`). Значит связка «пустая сумма → override из документа» — основной путь, не защита от редкого случая: не извлекли записку — строка молча выпала из расчёта. Проверка: для каждой целевой строки из `ledger.dirty` (пустая/непарсимая сумма) в фактах заёмщика обязан быть `amount_override` с этим `txn_id`; иначе провал `dirty_row_unrecovered` (строка молча потеряна). Тест: целевая dirty-строка без override → провал; с override → чисто. Фоновые dirty-строки в проверку не входят.
...
> **Граница со `eval/mutations_text.py` (отдельный инструмент, уже прогнан на публичном наборе, все 36/36).** Слой извлечения — мутация ТЕКСТА документа (rename / limits ×0.72 внутри сегмента пункта / clauses 6.x→7.x) со сверкой извлечённых спек и фактов против эталона, без прогона пайплайна — покрыт `mutations_text.py`. Задача 28 — сквозной слой: мутация ДАТАСЕТА + полный `solve.main`. Не дублировать rename-текст; задача 28 сосредоточена на том, чего текстовая мутация не может: (а) rename через весь пайплайн (ответы поячеечно неизменны), (б) сдвиг порога через прогон, (в) **EUR (обязательна) — единственная непроверенная ветка 5.5.1, нужен леджер+fx, а не только текст**. Мутация `clauses` (генерализация по номерам пунктов) остаётся за `mutations_text.py`.

**Files:**
- Create: `eval/mutations.py`
- Related (отдельный инструмент, не в этой задаче): `eval/mutations_text.py` — extraction-layer мутации rename/limits/clauses
...
**Правки по ревью (обязательны):**
- Ожидание слепых страниц в тесте: с правилом «И» (задача 18) на публичном наборе их **8** (в 5 документах) — в тесте `assert s["blind_pages"] == 8` вместо `>= 4`. Ранее здесь стояло «9» — описка при переносе из research-дока, где в таблице раздела 3 и в перечне документов раздела 2 стоит 8; реализация разобрана прямым замером (200 PDF, 843 страницы, слепые: `2ed0b2ee4b57` стр. 3–4, `57a1d81c8b17` стр. 7, `63e162bd710b` стр. 2, `aaf665cbc612` стр. 2, `f3fa6d20c8a1` стр. 1–3), а не подогнана под ожидание.
- Счётчик пограничных страниц (research-док, раздел 3) считается **раздельно по направлению**, а не одним числом: «мало символов, но числа есть» — 1 страница, «текста много, чисел мало» — 159 (титулы и оглавления). Сигнал про сканы несёт только первый; сложение утопило бы его в стабильном шуме.
...
- [ ] **Step 5: Чеклист репетиции 8 августа** (руками, по разделу 8 спеки + гейты ревью):
  ...
  - модель зафиксирована — `claude-haiku-4-5-20251001` (замер: 36/36 спек, 12/12 фактов, $1.49); бейк-офф не нужен, отметить фактическую латентность/стоимость в run-report.
