...
def test_extracted_full_run_beats_floor():
    ...
    # 33.00 — замер после отброса замененных редакций кумулятивных типов
    # (было 30.00: B1 6.1, B1 6.2 и P9 6.1 считались по предварительному
    # решению из рабочего документа). Порог поднимается тем же коммитом,
    # что и улучшение, иначе откат назад не поймается.
    assert total >= 33.00, f"извлечённый прогон просел: {total:.2f}"
