...
def test_extracted_full_run_beats_floor():
    ...
    # +4.00 (пять ячеек считались по предварительным решениям из замененных и
    # черновых рабочих документов), порог владения, применённый кодом, — ещё
    # +0.50. Порог теста поднимается тем же коммитом, что и улучшение, иначе
    # откат назад не поймается.
    assert total >= 34.50, f"извлечённый прогон просел: {total:.2f}"
