...
def test_extracted_full_run_beats_floor():
    ...
    # 34.00 — замер после отброса недействующих редакций кумулятивных типов
    # (было 30.00: пять ячеек считались по предварительным решениям из
    # замененных и черновых рабочих документов). Порог поднимается тем же
    # коммитом, что и улучшение, иначе откат назад не поймается.
    assert total >= 34.00, f"извлечённый прогон просел: {total:.2f}"
