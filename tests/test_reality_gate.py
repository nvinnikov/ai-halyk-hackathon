...
Форма — отчётная, а не блокирующая: поимённые сравнения помечены
xfail(strict=False), пока код не умеет считать ячейку — XFAIL, научился —
XPASS, и прогресс виден одной строкой сводки. Падать тесту есть от чего
ровно в двух случаях: прогон реплики сломался или разъехалась ФОРМА ответа —
это уже регрессия конвейера, не «код ещё не дорос».
...
@pytest.mark.xfail(strict=False, reason="код ещё не считает ячейку сам — см. роадмап в docs/ops")
