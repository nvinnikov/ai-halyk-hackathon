...
# 34.52 — было 30.00. Отброс недействующих редакций кумулятивных типов дал
# +4.00, порог владения, применённый кодом, — ещё +0.50, guard семьи метрики в
# ярусе эвристики — 0.02 (порог вместо посчитанных не тех долларов). Этот гейт,
# в отличие от floor-теста в test_extracted_run.py, не помечен маркером llm и
# идёт в make check и CI — то есть именно он и стережёт боевой путь от отката.
EXTRACTED_BASELINE = 34.52
