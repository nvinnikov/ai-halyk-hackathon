...
def test_verify_quote_survives_word_broken_by_layout():
    """Вёрстка PDF рвёт слово («У словием» — буквица первого слова абзаца,
    боевой прогон 2026-08-09): модель цитирует слово целым, пробельный матч
    не находит подстроку — работает пробельно-нечувствительный повтор."""
    source = (
        "Пункт 6.2. У словием предоставления заёмных средств является то, "
        "что поступления будут не ниже $7,200,000.00."
    )
    assert verify_quote("Условием предоставления заёмных средств", source)
    # Обратное направление той же вёрстки: разрыв в цитате, слитно в источнике.
    assert verify_quote("У словием предоставления", "Условием предоставления заёмных средств")


def test_verify_quote_despaced_does_not_admit_fabrication():
    """Повтор без пробелов не ослабляет защиту: выдуманный текст не совпадает
    и без пробелов, а фрагменты многоточия обязаны стоять по порядку в том же
    окне."""
    source = "Заёмщик обязуется поддерживать коэффициент покрытия не ниже 1.25x."
    assert not verify_quote("Заёмщик обязуется не допускать превышения", source)
    # Перестановка фрагментов не проходит и в беспробельной форме.
    assert not verify_quote("не ниже 1.25x … Заёмщик обязуется", source)
    # Односимвольные фрагменты многоточия отсекаются и в беспробельной форме.
    assert not verify_quote("З … 5", source)
