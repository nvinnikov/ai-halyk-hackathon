...
def test_unparse_roundtrip_on_templates_and_filters():
    ...
    фильтров: unparse кормит нормализацию EBITDA-подвыражений по определению
    из договора, и переписанный AST обязан вернуться в текст той же грамматики."""
    ...
    cases = list(TEMPLATES.values()) + [
        ...
        # Mul из процентных кэпов (#30) — unparse обязан знать все узлы грамматики.
        "min(add(agg(REVENUE, in), const(1)), mul(const(0.05), agg(REVENUE, in)))",
    ]
    for text in cases:
        ast = parse(text)
