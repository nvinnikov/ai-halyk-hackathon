...
# Порядок несущий: иглы покрытий стоят РАНЬШЕ revenue, потому что «покрытие …
# выручкой» содержит «выручк», и долларовый шаблон revenue при коэффициентном
# min-пороге — ровно та клетка, где guard семьи метрики по построению молчит
# (solve._family_mismatch ловит только превышение на max). Игла «процент», а не
# «процентн»: формулировка организаторов — «покрытия процентов», и суффиксная
# форма её не матчила — ступень icr для русского текста была мертва.
_KEYWORDS = [
    ("related_abs", ("related", "связанн", "аффилир")),
    ("capex", ("capital expenditure", "capex", "капитальн")),
    ("icr", ("interest cover", "процент", "icr")),
    ("insurance_cover", ("insurance", "страхов")),
    ("revenue_cover_payroll_utilities", ("покрыти", "cover")),
