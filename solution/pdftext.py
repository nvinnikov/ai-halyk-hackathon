...
def is_blind(text: str) -> bool:
    """«И», не «ИЛИ»: на текущем тексте (TEXT_VERSION=2, футер снят) «и» даёт
    8 слепых страниц, включая оба известных vision-кейса; «или» дало бы 168
    (8 + 160 пограничных — см. is_borderline и baseline sanity) — ложные это
    титулы и оглавления с нормальным текстом, но <3 числами. Старый замер до
    TEXT_VERSION=2 — 9 против 115. Ложный vision-вызов не просто дорог — он
    подменяет уже извлечённый текст ответом модели в route.full_text, то
    есть рискует точностью."""
    t = _normalize(text)
