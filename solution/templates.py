...
# Порог и требуемый отрыв стоят в зазоре между двумя замерами: переформулировка
# заголовка держит сходство со СВОИМ шаблоном заметно выше порога, а самая
# близкая пара РАЗНЫХ шаблонов — ниже. Сам зазор закреплён тестом
# (test_heading_similarity_separates_own_from_foreign): он и есть основание
# этих двух чисел, поэтому меняются они только вместе с ним.
