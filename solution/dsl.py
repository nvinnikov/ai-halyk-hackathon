...
def unparse(node) -> str:
    ...
    Нужен подмене шаблоном с переносом фильтров: cellspec несёт и AST, и текст
    (текст читают трейс и теневое сравнение), поэтому перестроенный AST обязан
    уметь вернуться в текст той же грамматики."""
