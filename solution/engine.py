...
def related_payments(rows, f):
    """Только расходные строки: некоторые связанные стороны могут являться
    источниками поступлений в исходных данных. Поступления исключаются из
    расчёта related_abs по определению метрики (платежи, а не поступления)."""
