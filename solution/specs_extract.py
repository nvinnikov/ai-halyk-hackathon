...
- limit: порог строкой (доли — числом: 7% => 0.07; кратности: 3.5x => 3.5);
- trigger: если тест применяется только при условии — условие как сравнение
  ...
  применение теста зависит от значения показателя (пример: «тест применяется,
  только если поступления по финансированию за период превысили указанную в
  пункте сумму» — springing-ковенант). Срок действия
...
def _limit_forms(limit: str) -> set[str]:
    """Цифровые формы порога для поиска внутри цитаты: 9.00/9.0/9,
    а для долей — ещё и процентная форма (0.04 ↔ 4%)."""
    try:
        d = Decimal(limit)
